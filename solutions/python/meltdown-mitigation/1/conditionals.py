...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    if efficiency >= 80.00:
        output = "green"
    elif 60.00 <= efficiency < 80.00:
        output = "orange"
    elif 30.00 <= efficiency < 60.00:
        output = "red"
    else:
        output = "black"
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    if (
        (threshold * 0 / 100) < criticality < (threshold * 10 / 100)
        or (threshold * 0 / 100) > criticality > (threshold * 10 / 100)
        or criticality == threshold
    ):
        status = "NORMAL"
    elif criticality < (threshold * 90 / 100):
        status = "LOW"
    else:
        status = "DANGER"
